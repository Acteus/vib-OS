...
int socket_accept(int sockfd, struct sockaddr *addr, unsigned int *addrlen)
{
    ...
    /* TODO: Block waiting for connection */
    /* For now, return error */
    
    printk(KERN_DEBUG "NET: Accept on socket %d (not implemented)\n", sockfd);
...
int socket_connect(int sockfd, const struct sockaddr *addr, unsigned int addrlen)
{
    ...
    /* TODO: Initiate TCP handshake for SOCK_STREAM */
    
    sock->state = SS_CONNECTED;
    
    printk(KERN_DEBUG "NET: Socket %d connected\n", sockfd);
...
ssize_t socket_send(int sockfd, const void *buf, size_t len, int flags)
{
    ...
    /* TODO: Actually send data via network interface */
    
    printk(KERN_DEBUG "NET: Send %zu bytes on socket %d\n", len, sockfd);
    
    return len;  /* Pretend we sent all data */
}
...
ssize_t socket_recv(int sockfd, void *buf, size_t len, int flags)
{
    ...
    /* TODO: Actually receive data from network */
    
    printk(KERN_DEBUG "NET: Recv up to %zu bytes on socket %d\n", len, sockfd);
...
int socket_close(int sockfd)
{
    ...
    /* TODO: Proper connection teardown for TCP */
    
    kfree(sock);
    socket_table[sockfd] = NULL;
    
    printk(KERN_DEBUG "NET: Socket %d closed\n", sockfd);
