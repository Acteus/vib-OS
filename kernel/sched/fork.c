...
{
    (void)arg;
    printk(KERN_DEBUG "fork_entry: child task starting\n");
}
...
long do_fork(unsigned long flags)
{
    ...
    printk(KERN_DEBUG "do_fork: parent PID %d\n", current_task->pid);
    ...
    printk(KERN_DEBUG "do_fork: created child PID %d\n", child->pid);
...
static int load_elf_binary(const char *path, uint64_t *entry_point)
{
    ...
    for (int i = 0; i < ehdr.e_phnum; i++) {
        ...
        for (size_t offset = 0; offset < size; offset += PAGE_SIZE) {
            ...
            if (phdr.p_flags & 0x1) vm_flags |= VM_EXEC;
            if (phdr.p_flags & 0x2) vm_flags |= VM_WRITE;
